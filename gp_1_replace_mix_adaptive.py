...
class GPPdf:
        ...
        while self.generation <= self.max_gen:
            ...
            for i in range(len(self.popul)):
                if i not in self.vid_from_trace:
                    ...
                    try:
                        #self.popul[i] = PdfGenome.mutation_with_trace_lessrand(self.ext_trie, self.popul[i], self.mut_rate, self.ext_genome)
                        #self.popul[i] = PdfGenome.mutation_with_trace_inswap(self.ext_trie, self.popul[i], self.mut_rate, self.ext_genome)
                        #self.popul[i] = PdfGenome.mutation_with_trace_lessrand(self.ext_trie, self.popul[i], self.mut_rate, self.ext_genome)
    ...
    def initial_population(self):
        ...
        while len(popul) < int(self.pop_size):
            ...
            root = PdfGenome.mutation_with_trace_swap(self.ext_trie, root, self.mut_rate, self.ext_genome)
            popul.append(root)
        return popul
