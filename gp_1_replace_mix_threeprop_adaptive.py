...
class GPPdf:
    def __init__(self,
        ...
        # Load exploit_spec
        self.exploit_spec = pickle.load(open('/home/yz/data/traintest_all_500test/exploit_spec/test_malicious.pickle', 'rb'))
        if self.seed_sha1 == 'c713d93780ffc164aec45e4cb6a0197087dc6d8f':
            self.exploit_source = '/Root/Names/JavaScript/Names'
        elif self.seed_sha1 in ['74f03105b9ab8a699d176b86cd97c0651f643cfd', 'e6d2727825ee8403538abdf8a97913af596ff08f', '745f3d57c7fb13061fc33610ceacd864c05c4b7f', '6b1e35d834aa10ff453a21b0a1e99864501731ad']:
            self.exploit_source = '/Root/OpenAction/JS'
        else:
            all_paths = self.exploit_spec[self.seed_sha1]
            if ['/Root', '/OpenAction', '/JS'] in all_paths or ['/Root', '/OpenAction'] in all_paths:
                self.exploit_source = '/Root/OpenAction/JS'
            elif ['/Root', '/Names', '/JavaScript', '/Names', 1, '/JS'] in all_paths or ['/Root', '/Names', '/JavaScript', '/Names', 1] in all_paths: 
                self.exploit_source = '/Root/Names/JavaScript/Names'
            elif self.seed_sha1 in set(['7d4604b87ae5b97e8411bc7a3ca22a0a15a4458f', '92cc5a76177d1ed7859ec993378e0452546de7cd', '965284e3359d30b840ec614b5b87d7a7896ce4fd', 'a414bcbffa46110f1483c1a31c1716de0c4303ef', 'dd64f160724dfd42335757858a825c28a07d42bd']):
                self.exploit_source = '/Root/Names/JavaScript/Names'
        ...
        while self.generation <= self.max_gen:
            ...
            for i in range(len(self.popul)):
                if i not in self.vid_from_trace:
                    ...
                    try:
                        self.popul[i] = PdfGenome.mutation_with_trace_swap_move(self.exploit_source, self.ext_trie, self.popul[i], self.mut_rate, self.ext_genome)
                    except Exception, e:
    ...
    def initial_population(self):
        ...
        while len(popul) < int(self.pop_size):
            ...
            #root = PdfGenome.mutation_noinsert(self.ext_trie, root, self.mut_rate, self.ext_genome)
            root = PdfGenome.mutation_with_trace_swap_move(self.exploit_source, self.ext_trie, root, self.mut_rate, self.ext_genome)
            popul.append(root)
        return popul
...
if __name__ == "__main__":
    ...
    if classifier_name == 'pdfrate':
        ...
        from lib.fitness import fitness_threeprop as fitness_func
    elif classifier_name == 'monotonic':
        from lib.fitness import fitness_monotonic as fitness_func
    elif classifier_name == 'baseline_adv':
        from lib.fitness import fitness_baseline_adv as fitness_func
