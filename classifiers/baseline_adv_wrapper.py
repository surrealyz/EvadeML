...
HOST = config.get('baseline_adv', 'host')
PORT = int(config.get('baseline_adv', 'port'))

def baseline_adv(pdf_paths):
    X = hidost_feature(pdf_paths, nfeat=3514, fpath='/home/yz/data/traintest_all_500test/features.nppf') 
    return query_tf(X.tolist())

def query_tf(X):
    payload = {'instances': X}
    REST_URL='http://%s:%d/v1/models/baseline_adv_combine_two:predict' % (HOST, PORT)
...
if __name__ == "__main__":
    REST_URL='http://localhost:8501/v1/models/baseline_adv_combine_two:predict'
